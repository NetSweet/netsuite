  ...
  module Records
    describe ItemFulfillment do
      context "unify package list attributes" do
        ...
        subject { described_class.new({ package_ups_list: package_ups_list }) }

        it "access ups attributes via package_list" do
          package = subject.package_list.packages.first
          expect(package.package_tracking_number).to eq package_ups_list[:package_ups][:package_tracking_number_ups]
        end
      end
    end
  end
